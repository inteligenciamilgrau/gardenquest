...
function isAiGameRoute(pathname = '') {
  return (
    pathname.startsWith('/api/v1/ai-game/') || pathname.startsWith('/api/v1/games/garden-quest/')
  );
}
...
function setupSecurity(app) {
  ...
  const cspConnectSrc = [
    "'self'",
    frontendOrigin,
    ...(config.CSP_ALLOW_LOCAL_CONNECT_SRC
  ...
  const generalLimiter = buildJsonRateLimiter({
    ...
    skip: (req) =>
      req.path === '/health' ||
      isAuthRoute(req.path) ||
      isAiGameRoute(req.path) ||
      isPlatformRoute(req.path),
  });
