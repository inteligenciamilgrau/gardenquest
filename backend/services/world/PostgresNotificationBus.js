...
function normalizeChannel(channel) {
  ...
  if (!/^[a-zA-Z0-9_]+$/.test(value))
    throw new Error(`Invalid Postgres notification channel: ${value || '<empty>'}`);
  return value;
}
...
class PostgresNotificationBus {
  constructor({
    channel,
    name = null,
    reconnectMs = config.WORLD_RUNTIME_BUS_RECONNECT_MS,
    logger = console,
  } = {}) {
  ...
  _scheduleReconnect() {
    ...
    this.reconnectHandle = setTimeout(() => {
      ...
      this._connect().catch((e) =>
        this.logger.error(`PG notify bus (${this.name}) reconnect failed:`, e.message)
      );
    }, this.reconnectMs);
  ...
  _handleEnd() {
    if (this.client) {
      this.client.removeListener('notification', this._handleNotification);
      this.client.removeListener('error', this._handleError);
      this.client.removeListener('end', this._handleEnd);
      this.client = null;
    }
    this.stats.connected = false;
  ...
  _handleError(error) {
    this.stats.lastError = error.message;
    this.logger.error(`PG notify bus (${this.name}) error:`, error.message);
  }
  ...
  _handleNotification(message) {
    ...
    this.stats.lastPayloadAt = new Date().toISOString();
    for (const handler of this.handlers) {
      Promise.resolve()
        .then(() => handler(payload, message))
        .catch((e) => this.logger.error(`PG notify bus (${this.name}) handler failed:`, e.message));
    }
  }
}
