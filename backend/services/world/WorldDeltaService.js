...
function indexActors(list) {
  ...
  for (const actor of Array.isArray(list) ? list : []) {
    if (actor?.id) map.set(String(actor.id), actor);
  }
  return map;
}
...
function computeActorDelta(previousActors, nextActors) {
  ...
  for (const [actorId, nextActor] of nextMap.entries()) {
    ...
    if (!prev || stableSerialize(prev) !== stableSerialize(nextActor))
      upsert.push(cloneJson(nextActor, {}));
  }
  for (const actorId of previousMap.keys()) {
    if (!nextMap.has(actorId)) removeIds.push(actorId);
  }
...
function buildSnapshotDelta(
  previousSnapshot,
  nextSnapshot,
  { selfPayload = undefined, runtimeMeta = undefined } = {}
) {
  const prev = previousSnapshot || {},
    next = nextSnapshot || {};
  const delta = {
    serverTime: next.serverTime || new Date().toISOString(),
  ...
  if (stableSerialize(prev.settings) !== stableSerialize(next.settings))
    delta.settings = cloneJson(next.settings, null);
  ...
  const worldDelta = {};
  if (stableSerialize(prevW?.trees) !== stableSerialize(nextW?.trees))
    worldDelta.trees = cloneJson(nextW?.trees, []);
  if (stableSerialize(prevW?.droppedApples) !== stableSerialize(nextW?.droppedApples))
    worldDelta.droppedApples = cloneJson(nextW?.droppedApples, []);
  if (stableSerialize(prevW?.swords) !== stableSerialize(nextW?.swords))
    worldDelta.swords = cloneJson(nextW?.swords, []);
  if (stableSerialize(prevW?.bows) !== stableSerialize(nextW?.bows))
    worldDelta.bows = cloneJson(nextW?.bows, []);
  if (stableSerialize(prevW?.arrows) !== stableSerialize(nextW?.arrows))
    worldDelta.arrows = cloneJson(nextW?.arrows, []);
  if (stableSerialize(prevW?.elevators) !== stableSerialize(nextW?.elevators))
    worldDelta.elevators = cloneJson(nextW?.elevators, []);
  if (stableSerialize(prevW?.graves) !== stableSerialize(nextW?.graves))
    worldDelta.graves = cloneJson(nextW?.graves, []);
  if (stableSerialize(prevW?.soccer) !== stableSerialize(nextW?.soccer))
    worldDelta.soccer = cloneJson(nextW?.soccer, null);
  ...
  if (stableSerialize(prev.leaderboard) !== stableSerialize(next.leaderboard))
    delta.leaderboard = cloneJson(next.leaderboard, null);
  if (stableSerialize(prev.soccerLeaderboard) !== stableSerialize(next.soccerLeaderboard))
    delta.soccerLeaderboard = cloneJson(next.soccerLeaderboard, null);
...
function actorLabel(actor) {
  return actor?.actorType === 'agent' ? `🤖 ${actor.name || 'Agente'}` : actor?.name || 'Jogador';
}
...
function buildRuntimeEvents(previousSnapshot, nextSnapshot, snapshotVersion) {
  ...
  const prevActors = indexActors(prev.players),
    nextActors = indexActors(next.players);
  for (const [actorId, actor] of nextActors.entries()) {
    if (!prevActors.has(actorId))
      events.push({
        eventType: 'actor_joined',
        visibility: 'public',
        actorId,
        actorType: actor.actorType || 'player',
        snapshotVersion,
        payloadJson: {
          actorId,
          actorType: actor.actorType || 'player',
          ...
          joinedAt: next.serverTime || new Date().toISOString(),
        },
      });
  }
  for (const [actorId, actor] of prevActors.entries()) {
    if (!nextActors.has(actorId))
      events.push({
        eventType: 'actor_left',
        visibility: 'public',
        actorId,
        actorType: actor.actorType || 'player',
        snapshotVersion,
        payloadJson: {
          ...
          actorName: actor.name || 'Jogador',
          label: actorLabel(actor),
          leftAt: next.serverTime || new Date().toISOString(),
        },
      });
  }
  ...
  (Array.isArray(next.playerChat?.entries) ? next.playerChat.entries : [])
    ...
    .forEach((e) =>
      events.push({
        ...
        visibility: 'public',
        actorId: e.playerId || null,
        actorType: 'player',
        snapshotVersion,
        payloadJson: cloneJson(e, {}),
      })
    );
  ...
  if ((Number(nextGoalEvent?.sequence) || 0) > prevGoalSeq)
    events.push({
      eventType: 'soccer_goal',
      visibility: 'public',
      actorId: nextGoalEvent?.scorerActorId || null,
      actorType: nextGoalEvent?.scorerActorType || 'player',
      snapshotVersion,
      payloadJson: cloneJson(nextGoalEvent, {}),
    });
  if (stableSerialize(prev.leaderboard) !== stableSerialize(next.leaderboard))
    events.push({
      ...
      snapshotVersion,
      payloadJson: {
        updatedAt: next.leaderboard?.updatedAt || null,
        entries: cloneJson(next.leaderboard?.entries, []).slice(0, 5),
      },
    });
  return events;
}
